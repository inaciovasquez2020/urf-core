...
lean_path = ROOT / "lean" / "URF" / "Frontier" / "ArithmeticSpectralCoercivityBridgeToInputLemma.lean"
urf_path = ROOT / "lean" / "URF.lean"

if not lean_path.exists():
    raise SystemExit("MISSING_OBJECT := lean/URF/Frontier/ArithmeticSpectralCoercivityBridgeToInputLemma.lean")
...
for forbidden_claim in [
    "ARITHMETIC_SPECTRAL_COERCIVITY_BRIDGE_TO_INPUT_ANALYTIC_BRIDGE_CLOSED : Bool := true",
    "ARITHMETIC_SPECTRAL_COERCIVITY_BRIDGE_TO_INPUT_FINAL_CLOSURE_CLAIMED : Bool := true",
]:
    if forbidden_claim in text:
        raise SystemExit(f"BOUNDARY := forbidden closure claim present: {forbidden_claim}")

import_line = "import URF.Frontier.ArithmeticSpectralCoercivityBridgeToInputLemma"
if import_line not in urf_text.splitlines():
    raise SystemExit("MISSING_OBJECT := import URF.Frontier.ArithmeticSpectralCoercivityBridgeToInputLemma")

print("ARITHMETIC_SPECTRAL_COERCIVITY_BRIDGE_TO_INPUT_LEMMA_OK")
