...
EXPECTED_POLICY = {
    ...
    "status": "RUNTIME_VERIFY_ONLY_EXTERNAL_ISSUER_NOT_YET_DEPLOYED",
    ...
    "certified_property": (
        "checked TSA runtime surface is verify-only: no local signing command or "
        "private-key input is permitted, and only the pinned public verification "
        "key is present"
    ),
}
...
def test_external_identity_candidate_exists_but_is_not_promoted_to_issuer() -> None:
    policy = _policy()
